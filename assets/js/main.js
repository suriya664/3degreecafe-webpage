(function () {
  ...
  const positionNavDropdown = (toggle, nav) => {
    const width = window.innerWidth;
    // Skip positioning if nav is full width (between 768px and 960px breakpoint)
    if (width > 960) return; // Desktop view
    if (width <= 960 && width > 768) return; // Full width nav, no positioning needed
    
    const headerContainer = toggle.closest(".header-container");
    if (!headerContainer) return;
    
    const toggleRect = toggle.getBoundingClientRect();
    const containerRect = headerContainer.getBoundingClientRect();
    
    // Calculate right position to align dropdown with hamburger button
    const rightOffset = containerRect.right - toggleRect.right;
  ...
  navToggles.forEach((toggle) => {
    ...
    // Position dropdown on toggle click
    toggle.addEventListener("click", (event) => {
      event.stopPropagation();
      const isOpen = nav.classList.contains("is-open");
      closeNavMenus();
      if (!isOpen) {
        positionNavDropdown(toggle, nav);
